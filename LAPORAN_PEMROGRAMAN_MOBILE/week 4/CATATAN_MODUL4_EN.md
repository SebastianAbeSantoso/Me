...
Comparing my attempt: https://github.com/SebastianAbeSantoso/Me/blob/main/LAPORAN_PEMROGRAMAN_MOBILE/week%204/exercise_5.png with the mentor's was interesting, a bit of the names and structures were different, mainly was i didn't put footnotes and combined it into a single shared box, also the "free" part i put into menu, the mentor did not, so that's also the difference in the hierarchy. After trying to make the hierarchy in the files, there are a couple jarring mistakes, like there are two "MenuCatagories" and they're misspelled from "MenuCategories", theres also the folder hierarchy that i tried comparing, and it also has the same mistake as the previous one: https://github.com/SebastianAbeSantoso/Me/tree/main/LAPORAN_PEMROGRAMAN_MOBILE/week%204/fullsnack_exercise_5_6
...
I... lowkirkenuenly do not have an idea on how to solve this, because i do not even have an inkling of knowledge on Next.js so i just learned by following the answer given by the mentor, there are a couple of interesting things the mentor mentions, like in Next.js and similar frameworks there are methods to cache so subsequent fetches dont require networks since it's already cached, theres also the mentor's rule of thumb where he makes the function call from the ratings feature for data, rather than component for the decoupling. Anyways, here's the result: https://github.com/SebastianAbeSantoso/Me/tree/main/LAPORAN_PEMROGRAMAN_MOBILE/week%204/fullsnack_exercise_5_6, seek the /restaurant folder as thats where most of the exercise happens

### Chapter 36

#### 11 Maret 2026

The initial version of fullsnack may have a very clean architecture and design, but as the codebase grows, it's likely it will grow messy and complex, so it's important to spend some time defining guardrails and constraints, especially at the beginning of the project. The mentor shares a couple of tools to help with this, first one is [Dependency Cruiser](https://github.com/sverweij/dependency-cruiser), a library that works on js or ts that works to validate the dependancies of the codebase against a set of rules. An example is that modules should not depend on eachother, that's the point of dividing them into modules in the first place. Another one is [commonality](https://www.commonality.co/), which is a newer tool that's used in mono repos, that produces similar level of constraints and checks your mono repos. If you care about performance, there are a few things that can help to protect your architecture in terms of performance, like [Bundlesize](https://www.npmjs.com/package/bundlesize), which checks the bundles and the maximum size. If the performance is more strict, then there's called [performance budgets](https://www.speedcurve.com/blog/performance-budgets/), a way to check core web vitals and other metrics like size of bundles, images, etc. Last tool is a vscode extension called [SonarLint](https://marketplace.visualstudio.com/items?itemName=SonarSource.sonarlint-vscode), its more related to design, but it helps manage the complexity of your code. An article on cognitive complexity is this [article](https://gitnation.com/contents/conquering-complexity-refactoring-javascript-projects?timecode), it also shows some usage of the SonarLint extension.
